#!/usr/bin/env bash
#
# Post-deploy smoke test. Probes the running stack through the proxy
# port, asserts the three things that "the deploy worked" hinges on:
#
#   - /api/health is reporting status ok or degraded (anything else
#     means the DB is broken or the process is down)
#   - a verbatim APL query (⎕IO) lands the canonical Index Origin
#     page at top-1 in hybrid mode (catches "container is up but
#     pointed at an empty DB" and "embedder is unreachable from
#     web replicas")
#   - /api/version is reachable and reports a non-empty build_version
#     (catches "image deployed but binary is the old one")
#
# Usage:
#   ./deploy/verify.sh                # against http://localhost:8080
#   HOST=docsearch.example HOST_PORT=80 ./deploy/verify.sh
...
# 2. canonical search. Retry a few times to tolerate embedder
# cold-start (typically ~10 s on first ever call against a fresh
# container).
# ⎕IO is %E2%8E%95%49%4F percent-encoded.
search=""
